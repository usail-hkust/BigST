...
class bigst(nn.Module):
    def __init__(self, device, seq_num, in_dim, hid_dim, num_nodes, tau, random_feature_dim, node_emb_dim, time_emb_dim, \
                 use_residual, use_bn, use_spatial, dropout, supports=None, edge_indices=None):
        ...
        self.W_1 = nn.Conv2d(node_emb_dim+time_emb_dim*2+hid_dim, hid_dim, kernel_size=(1, 1), bias=True)
        self.W_2 = nn.Conv2d(node_emb_dim+time_emb_dim*2+hid_dim, hid_dim, kernel_size=(1, 1), bias=True)
    ...
    def forward(self, x):
        ...
        node_vec1 = self.W_1(x) # (B, dim, N, 1)
        node_vec2 = self.W_2(x) # (B, dim, N, 1)

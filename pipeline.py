...
class train_pipeline():
    def __init__(self, scaler, seq_num, in_dim, hid_dim, num_nodes, tau, random_feature_dim, node_emb_dim, time_emb_dim, \
                 use_residual, use_bn, use_spatial, dropout, lrate, wdecay, device, supports, edge_indices):
        self.model = bigst(device, seq_num, in_dim, hid_dim, num_nodes, tau, random_feature_dim, node_emb_dim, time_emb_dim, \
                           use_residual, use_bn, use_spatial, dropout, supports=supports, edge_indices=edge_indices)
    ...
    def train(self, input, real_val):
        ...
        if self.use_spatial:
            output, spatial_loss = self.model(input)
    ...
    def eval(self, input, real_val, flag='overall'):
        if flag=='overall':
            self.model.eval()
            output, _ = self.model(input)
            real = self.scaler.inverse_transform(real_val)
